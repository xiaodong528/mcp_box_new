...
- **架构支持**: x86_64/amd64 (生产服务器架构)

**⚠️ 重要说明**:
- 本项目镜像已针对 **x86_64 (amd64)** 架构构建，适用于绝大多数生产服务器
- 如果您在 ARM 架构的机器（如 Apple Silicon Mac）上构建，脚本会自动进行跨架构构建
- 部署包在 x86 和 ARM 架构的服务器上都可以正常部署和运行
...
- ✅ 构建 Docker 镜像 (强制使用 linux/amd64 平台)
- ✅ 拉取 x86_64 架构的 Python 基础镜像
- ✅ 保存镜像为 tar 包
- ✅ 打包所有必需文件
- ✅ 创建 `memo-offline-deploy.tar.gz` (约 1GB)

**📝 架构说明**:
- 脚本会自动构建 **x86_64/amd64** 架构的镜像，即使您在 ARM 架构的机器（如 Apple Silicon Mac）上运行
- 构建过程使用 Docker 的跨平台编译功能（QEMU 模拟）
- 在 ARM 机器上构建 x86 镜像可能需要较长时间（5-15 分钟）
- 生成的镜像可以在标准的 x86 生产服务器上原生运行，无需模拟

**⚡ 性能提示**:
- 如果您有 x86 机器可用，建议在 x86 机器上构建以获得更快的构建速度
- 跨架构构建时，`--no-cache` 参数确保完全重新编译
...
# 验证镜像架构 (可选)
docker images | grep -E "memo|python:3.12-slim"

# 检查镜像平台架构
docker inspect memo-memo:latest | grep Architecture
docker inspect memo-mcp-box:latest | grep Architecture
# 应该显示: "Architecture": "amd64"
```

**预期输出**:
```
memo-offline-deploy.tar.gz  约 900MB - 1.2GB
Architecture: amd64  (确认为 x86_64 架构)
```
...
**症状**: 容器启动时报 "exec format error" 或架构不兼容
...
3. 验证镜像平台
   ...
   ```bash
   # 查看镜像详细信息
   docker image inspect memo-memo:latest --format='{{.Os}}/{{.Architecture}}'
   # 应显示: linux/amd64
   ```

**解决方案**:

如果镜像架构与服务器不匹配，需要重新构建：
...
# 清理旧镜像
docker rmi memo-memo:latest memo-mcp-box:latest

# 重新运行准备脚本（会强制构建 x86_64 架构）
bash scripts/prepare-offline-images.sh
